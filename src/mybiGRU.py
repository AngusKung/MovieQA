...
def twoGRUmodel(args, maxlen, maxlen_pass, dim_glove):
	#modify by Hsieh 0620
	#maxlen_pass = maxlen
	dim_gru = args.dim_gru
	dim_proj = 300
	shared_linear_proj = CustomTimeDistributedDense(output_dim = dim_proj, input_dim = dim_glove)
	shared_GRU =  GRU(output_dim = dim_gru, dropout_W=args.dropout, return_sequences = False, input_shape = (maxlen,dim_proj), init = 'glorot_uniform', inner_init = 'orthogonal', inner_activation = 'hard_sigmoid')
	shared_backGRU = GRU(output_dim = dim_gru,dropout_W=args.dropout ,go_backwards=True, return_sequences = False, input_shape = (maxlen,dim_proj), init = 'glorot_uniform', inner_init = 'orthogonal', inner_activation = 'hard_sigmoid')
	passGRU =  GRU(output_dim = dim_gru, dropout_W=args.dropout, return_sequences = True, input_shape = (maxlen_pass,dim_proj), init = 'glorot_uniform', inner_init = 'orthogonal', inner_activation = 'hard_sigmoid')
	passbackGRU = GRU(output_dim = dim_gru,dropout_W=args.dropout ,go_backwards=True, return_sequences = True, input_shape = (maxlen_pass,dim_proj), init = 'glorot_uniform', inner_init = 'orthogonal', inner_activation = 'hard_sigmoid')


	pass_input = Input(shape=(maxlen_pass,dim_glove), dtype='float32', name='pass_input')
	pass_proj = shared_linear_proj(pass_input)
	pass_gru = passGRU(pass_proj)
	pass_backgru = passbackGRU(pass_proj)
	pass_con = merge([pass_gru,pass_backgru],mode='concat') # maxlen_pass, 2*dim_gru

	ques_input = Input(shape=(maxlen,dim_glove), dtype='float32', name='ques_input')
	ques_proj = shared_linear_proj(ques_input)
	ques_gru = shared_GRU(ques_proj)
	ques_backgru = shared_backGRU(ques_proj)
	ques_con = merge([ques_gru,ques_backgru],mode='concat') # , 2*dim_gru
	...
	A1_proj = shared_linear_proj(A1_input)
	a1_gru = shared_GRU(A1_proj)
	a1_backgru = shared_backGRU(A1_proj)
	a1_con = merge([a1_gru,a1_backgru],mode='concat') # , 2*dim_gru

	A2_proj = shared_linear_proj(A2_input)
	a2_gru = shared_GRU(A2_proj)
	a2_backgru = shared_backGRU(A2_proj)
	a2_con = merge([a2_gru,a2_backgru],mode='concat') # , 2*dim_gru

	A3_proj = shared_linear_proj(A3_input)
	a3_gru = shared_GRU(A3_proj)
	a3_backgru = shared_backGRU(A3_proj)
	a3_con = merge([a3_gru,a3_backgru],mode='concat') # , 2*dim_gru

	A4_proj = shared_linear_proj(A4_input)
	a4_gru = shared_GRU(A4_proj)
	a4_backgru = shared_backGRU(A4_proj)
	a4_con = merge([a4_gru,a4_backgru],mode='concat') # , 2*dim_gru

	A5_proj = shared_linear_proj(A5_input)
	a5_gru = shared_GRU(A5_proj)
	a5_backgru = shared_backGRU(A5_proj)
	a5_con = merge([a5_gru,a5_backgru],mode='concat') # , 2*dim_gru
